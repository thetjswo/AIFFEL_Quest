...
class PomodoroTimer {
  ...
  void start() {
    ...
    timer = Timer.periodic(Duration(seconds: 1), (Timer timer) {
      if (_current_duration > 0) { // 남은 시간이 있을 경우
        print('flutter: ${_current_duration ~/ 60}:${_current_duration % 60}'); // 남은 시간 출력
        _current_duration--; // 남은 시간 1초 감소
      } else { // 남은 시간이 없을 경우 (0초)
        if (_is_working) { // 현재 상태가 작업 중인 경우
          if (round % 4 == 0) _current_duration = last_break_duration; // 4회차일 경우 긴 휴식 시간 설정
          else _current_duration = break_duration; // 그 외 회차일 경우 일반 휴식 시간 설정

          _is_working = false; // 상태를 휴식으로 전환
          print('작업 시간이 종료되었습니다. 휴식 시간을 시작합니다.');
        } else { // 현재 상태가 휴식 중인 경우
...
PROBLEM
- 코딩 적인 부분에서 아직 언어에 대한 이해도가 낮아서 직접 코딩하는 시간보다는 말로 부탁하는 시간이 많아다.
